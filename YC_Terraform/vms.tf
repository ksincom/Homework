...
resource "local_file" "inventory" {
  ...
  [webservers:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -i /home/ks/.ssh/id_rsa -p 22 -W %h:%p -q ks@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'

  [all:vars]
  ansible_user=ks
  ansible_ssh_private_key_file=/home/ks/.ssh/id_rsa
  XYZ
  filename = "./hosts.ini"
}