...
Ответ:
Docker Compose необходим когда требуется работа с несколькими контейнерами без длинных команд, которые нужно запоминать или переписывать, ручного управления зависимостями и постоянной путаницы с сетями и томами. 
Docker Compose решает это тем, что позволяет описать все сервисы моего приложения в одном понятном YAML-файле, а затем запускать и управлять ими одной командой docker compose up.
Для меня лично это значит, что я могу легко запустить проекта из сохраненного docker-compose.yml в репозитории.
При разработке я легко смогу тестировать разные версии сервисов, смогу изолировать проекты друг от друга.
Такой подход превращает управление инфраструктурой в простую и предсказуемую задачу.
